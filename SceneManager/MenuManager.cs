...
//We are going to have a page full of menus
//This i not ideal for an rpg but we wont have to many menus to use

namespace SceneManager
{
    public class MenuManager
    {
        List<string> animationType, linkType, linkID, menuItems;
        List<List<string>> attributes, contents;
        List<Texture2D> menuImages;//Texture2D in case you want to have images in the menu items instead of text
        ...
        //We want to make all the menu items equal in size because we dont know which is longer or shorter
        ...
        private void SetAnimations()
        {
            ...
            if (align.Contains("Center"))
            {
                ...
                {
                    //This will give us the startig center position for the Y co ordinate
            ...
            for (int i = 0; i < menuItems.Count; i++)
            {
                ...
                //for every menu item we have we are going to align it according to the center
                if (axis == 1)
                    pos.Y = (ScreenManager.Instance.Dimensions.Y - dimensions.Y) / 2;
                else
                    pos.X = (ScreenManager.Instance.Dimensions.X - dimensions.X) / 2;

                for (int j = 0; j < animationType.Count; j++)
                {
                    switch (animationType[j])
                    {
                        case "Fade":
                ...
                for (int j = 0; j < attributes[i].Count; j++)
                {
                    switch (attributes[i][j])
                    {
                        ...
                        case "Image":
                            ...
                            break;
                        case "Axis"://the .Parse changes it to an integer type
                            axis = int.Parse(contents[i][j]);
                            break;
                        case "Position"://Going to seperate position elements and position based on spaces
                            string[] temp = contents[i][j].Split(' ');
                            position = new Vector2(float.Parse(temp[0]), float.Parse(temp[1]));
                            break;
                        case "Source"://If we have a sprite sheet and only want to crop a certain place in teh image we can use this like a sourcerect
                            temp = contents[i][j].Split(' ');
                            source = new Rectangle(int.Parse(temp[0]), int.Parse(temp[1]), int.Parse(temp[2]), int.Parse(temp[3]));
                            break;
                        case "Animation":
                            animationType.Add(contents[i][j]);
                            break;
                        case "Align":
                            align = contents[i][j];
                            break;
                        case "LinkType"://The below two are how we are going to link to different things
        ...
        public void Update(GameTime gameTime, InputManager inputManager)
        {
            ...
            if (inputManager.KeyPressed(Keys.Enter, Keys.Space))
            {
                ...
                {
                    //Namespace plus the link id
                    //Our namespace for the title screen class will be scenemanager. whatever
                    //but if you set different namespaces for the screens then change the link id
                    //here we are creating a new type we get the type name from the namespaces and store it in new class
                    //then cast it to a gamescreen type and add it to the add screen
                    
                    Type newClass = Type.GetType("SceneManager." + linkID[itemNumber]);
            ...
            if (itemNumber < 0)
                itemNumber = 0;
            else if (itemNumber > menuImages.Count - 1)
